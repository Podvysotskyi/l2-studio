...
public static class StudioMessagingConfigurationExtensions
{
    ...
    private static void RouteControlMessages(WolverineOptions options)
    {
        ...
        options.PublishMessage<ImportC1NpcSpawns>().ToPostgresqlQueue(ControlQueue);
        ...
        options.PublishMessage<ImportC1ItemHandlers>().ToPostgresqlQueue(ControlQueue);
        options.PublishMessage<ImportC1ItemSkillTypes>().ToPostgresqlQueue(ControlQueue);

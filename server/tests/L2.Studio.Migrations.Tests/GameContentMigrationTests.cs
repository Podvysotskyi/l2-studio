    ...
    private GameContentDbContext CreateContentContext()
    {
        var options = new DbContextOptionsBuilder<GameContentDbContext>()
            .UseNpgsql(
                ...
                configuration =>
                {
                    configuration.MigrationsAssembly(typeof(MigrationAssemblyMarker).Assembly.FullName);
                    configuration.MigrationsHistoryTable("__EFMigrationsHistory", GameContentDbContext.SchemaName);
                })
            .Options;

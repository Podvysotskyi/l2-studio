...
async function loadCatalog() {
  try {
    catalog.value = await getAssetCatalog<LevelCatalogEntry>('levels', {
      pageSize: 500
    })
  ...
  try {
    previewCatalog.value =
      await getAssetCatalog<LevelPreviewCatalogEntry>('levelpreviews', {
        pageSize: 500
      })
  } catch {
...
async function loadJobs(schedule = true) {
  ...
  try {
    jobs.value = await getAssetImportJobs('levels')
    previewJobs.value = await getAssetImportJobs('levelpreviews')

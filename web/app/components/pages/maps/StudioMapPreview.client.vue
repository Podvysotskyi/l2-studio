...
import {
  ...
  unrealForward,
  unrealVector
} from '../../../utils/unreal-transform'
import {
  createTerrainMaterial,
  type TerrainMaterialController
} from '../../../utils/terrain-material'
...
const mapLights = new Map<
  string,
  { source: MapLightManifestEntry; rendered: Light }
>()
...
function applyLightGizmoSelection() {
  for (const [name, gizmo] of lightGizmos) {
    ...
    const color = mapLights.get(name)?.rendered.diffuse ?? Color3.White()
...
function syncLightGizmos() {
  ...
  for (const [name, { rendered }] of mapLights) {
...
function createMapLights() {
  ...
  for (const source of sources) {
    ...
    mapLights.set(source.name, { source, rendered })
  }
...
function focusLight(name: string) {
  if (!scene) return
  const entry = mapLights.get(name)
  if (!entry) {
...
async function loadBspEntry(bsp: MapBspMeshManifestEntry, version: number) {
...
async function loadMap() {
  ...
  scene.lights
    .filter((light) => light.name !== 'map-preview-light')
        ...
        batch.map(async (actor) => {
          try {
            ...
            if (!failedActorMeshUrls.has(actor.meshUrl!)) {
              ...
              console.warn(
                `Unable to load map actors using ${actor.meshUrl}.`,
                error
              )
            }
          }
...
onMounted(() => {
  ...
  scene.clearColor.copyFrom(inspectorClearColor)
  highlightLayer = new HighlightLayer('map-preview-highlight', scene)

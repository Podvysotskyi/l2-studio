...
import { buildLevelWorldGrid } from '../../../utils/level-world-grid'
...
async function loadCatalog() {
  try {
    catalog.value = await $fetch(
      assetCatalogUrl(apiBase, 'levels', { pageSize: 500 })
    )
  ...
  try {
    previewCatalog.value = await $fetch(
      assetCatalogUrl(apiBase, 'levelpreviews', { pageSize: 500 })
    )
  } catch {
...
async function loadJobs(schedule = true) {
  ...
  try {
    jobs.value = await $fetch<AssetImportJob[]>(
      assetImportsUrl(apiBase, 'levels'),
      { query: { limit: 20 } }
    )
    previewJobs.value = await $fetch<AssetImportJob[]>(
      assetImportsUrl(apiBase, 'levelpreviews'),
      { query: { limit: 20 } }
    )
...
async function queuePreviews(levelName?: string) {
  ...
  jobsError.value = undefined
  try {
    await $fetch(assetImportsUrl(apiBase, 'levelpreviews'), {
      method: 'POST',
...
async function queueImport() {
  ...
  jobsError.value = undefined
  try {
    await $fetch(assetImportsUrl(apiBase, 'levels'), {
      method: 'POST'
    })
    await loadJobs()
  } catch {

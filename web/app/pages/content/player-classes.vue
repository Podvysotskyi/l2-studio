...
const roots = computed(() => buildPlayerClassHierarchy(records.value))
...
const matchCount = computed(() => {
  ...
  if (!term) return records.value.length
...
async function loadPlayerClasses() {
  loading.value = true
  error.value = undefined
  try {
    records.value = await $fetch<PlayerClassRecord[]>(
      playerClassDirectoryUrl(apiBase)
    )

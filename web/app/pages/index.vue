...
async function loadContentSummary() {
  try {
    const [
      ...
      $fetch<NpcPage>(
        npcDirectoryUrl(apiBase, { page: 1, pageSize: 1 })
      ),
      $fetch<SkillPage>(
        skillDirectoryUrl(apiBase, { page: 1, pageSize: 1 })
      ),
  ...
  try {
    const results = await Promise.all(
      (
        ...
        $fetch<AssetImportJob[]>(assetImportsUrl(apiBase, kind), {
          query: { limit: 10 }
        })
      )
    )

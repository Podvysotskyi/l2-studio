...
import { filterMapActors } from '../../app/utils/map-actors'
import { paginate } from '../../app/utils/directory'

const actors: MapActorManifestEntry[] = [
  {
...
describe('Map actor instances', () => {
  it('searches actor names, classes, packages, and mesh objects', () => {
    expect(filterMapActors(actors, 'actor12')).toEqual([actors[0]])
    expect(filterMapActors(actors, 'decoration')).toEqual([actors[1]])
    expect(filterMapActors(actors, 'VILLAGE')).toEqual([actors[0]])
    expect(filterMapActors(actors, 'tower')).toEqual([actors[0]])
  })

  it('returns all actors for an empty query without changing their order', () => {
    expect(filterMapActors(actors, '  ')).toEqual(actors)
  })
